...
int main() {
    ...
    while (1) {
        ...
        if (fork() == 0) {
            ...
            mktime(t);
            char dateStings[10];
            strftime(dateStings, 10, "%d%m%y", t);
            char timeStings[10];
            strftime(timeStings, 10, "%H%M%S", t);
            FILE *logs = fopen("AccessLog.txt", "a");
            fprintf(logs, "%s:%s:%s:%d:%s:%s\n", dateStings, timeStings, inet_ntoa(client_addr.sin_addr),
            ...
            if (strcmp(method, "GET") == 0) {
                ...
                struct tm *last;
                struct tm ifModifiedSince;
                strptime(REQ_HEADERS[IF_MODIFIED_SINCE].value, "%a, %d %b %Y %T GMT", &ifModifiedSince);
                if (stat(url, &fileInfo) == 0) {
                    last = gmtime(&fileInfo.st_mtime);
                    strftime(LastModified, 80, "%a, %d %b %Y %T GMT", last);
                } else {
                ...
                time_t lastModifiedTime = mktime(last);
                time_t ifModifiedSinceTime = mktime(&ifModifiedSince);
